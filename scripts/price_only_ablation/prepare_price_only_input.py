...
from price_only_ablation.build_model_input import (
    build_flattened_price_only_input_from_csv,
    save_price_only_input_npz,
)
...
def run(config_path: Path) -> None:
    ...
    arrays = build_flattened_price_only_input_from_csv(
        aligned_csv_path=aligned_csv,
        target_col=target_col,
        normalize_price_per_company=normalize_prices,
    )
    save_price_only_input_npz(arrays, out_npz)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare model-ready flattened inputs for price-only training")
    parser.add_argument(
        "--config",
        type=Path,
